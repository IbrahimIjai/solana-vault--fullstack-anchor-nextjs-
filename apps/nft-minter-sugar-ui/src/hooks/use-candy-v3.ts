...
import { fetchMintLimit, guardToPaymentUtil, mergeGuards, parseGuardGroup, parseGuardStates } from './utils'
...
export default function useCandyMachineV3(
  ...
  const proofMemo = React.useMemo(() => {
    ...
    const merkles: { [k: string]: { tree: MerkleTree; proof: Uint8Array[] } } = candyMachineOpts.allowLists.reduce(
